...
c
      ...
      do i = 1, nkey
         ...
         if (keyword(1:4) .eq. 'VDW ') then
            ...
            if (k.ge.1 .and. k.le.maxclass) then
               ...
               read (string,*,err=10,end=10)  rd,ep,rdn,lp
   10          continue
   ...
   30                format (/,' Additional van der Waals Parameters :',
     ...
     &                          'Lee-Ping Factor'/)
               ...
               lpf(k) = lp
               if (.not. silent) then
                  write (iout,40)  k,rd,ep,rdn,lp
   40             format (4x,i6,8x,2f12.4,f12.3,f12.3)
               end if
...
c
      ...
      if (allocated(lpfactor))  deallocate (lpfactor)
      ...
      allocate (lpfactor(maxclass,maxclass))
