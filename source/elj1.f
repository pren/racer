...
c     ##################################################################
c     ##                                                              ##
c     ##  subroutine elj1_lp  --  double loop Lennard-Jones vdw derivs  ##
c     ##                                                              ##
c     ##################################################################
c
c
c     "elj1_lp" calculates the Lee-pings Lennard-Jones 6-12 van der Waals energy
...
!$OMP& jvdw,xred,yred,zred,use,n12,n13,n14,n15,
...
c ### sigv converts radmin to sigma to use in LP vdw functional form
c ### LP uses sigma in paper, want to be consistent
                  sigv = rv / twosix

                  lpterm = (3 / (lpfact + 3))
                  e = (2 * eps / (1 - lpterm))
     &                          * (sigv**6/(sigv**6 + rik2**3))
     &                          * (lpterm * exp(lpfact*(1-(rik/sigv))) 
     &                          - 1)
                  de = (-2*eps/(1-lpterm)) * (6*sigv**6 * rik**5)
     &                           / (sigv**6 + rik**6)**2
     &                           *(lpterm * exp(lpfact*(1-rik/sigv))
     &                           -1)
     &                           +(-2*eps/(1-lpterm))
...
c                     p4 = (rv/rik)**4
c                     e = eps * (p4**2 - 2*p4)
c     &                         * exp(-(rik-rv)**2/lpfact**2)
c                     de = 8*eps*p4*rik**(-1) * (-p4 + 1)
c     &                         * exp(-(rik-rv)**2/lpfact**2)
c     &                         + eps * (p4**2 - 2*p4)
c     &                         * (-2*(rik-rv)/lpfact**2)
c     &                         * exp(-(rik-rv)**2/lpfact**2)
...
c ### LP uses sigma in paper, want to be consistent
     ...
     &                          - 1) 
                     de = (-2*eps/(1-lpterm)) * (6*sigv**6 * rik**5)
     &                           /(sigv**6 + rik**6)**2
     &                           *(lpterm * exp(lpfact*(1-rik/sigv))-1)
     &                           +(-2*eps/(1-lpterm))*(sigv**6/(sigv**6
     &                           + rik**6)) * (lpterm*(lpfact/sigv)
     &                           *exp(lpfact*(1-rik/sigv)))
