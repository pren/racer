...
!$OMP& x,y,z,f,pchg,n12,n13,n14,n15,i12,i13,i14,
!$OMP& i15,c2scale,c3scale,c4scale,c5scale,use_group,use_bounds,
!$OMP& off2,molcule,ebuffer,kdebyescreen) 
!$OMP& firstprivate(cscale)
!$OMP& shared(eco,eintero,deco,viro)
!$OMP DO reduction(+:eco,eintero,deco,viro)
!$OMP& schedule(guided)
c
...
c
                  eco = eco + e
                  deco(1,i) = deco(1,i) + dedx
                  deco(2,i) = deco(2,i) + dedy
                  deco(3,i) = deco(3,i) + dedz
                  deco(1,ic) = deco(1,ic) + dedxc
                  deco(2,ic) = deco(2,ic) + dedyc
                  deco(3,ic) = deco(3,ic) + dedzc
                  deco(1,k) = deco(1,k) - dedx
                  deco(2,k) = deco(2,k) - dedy
                  deco(3,k) = deco(3,k) - dedz
                  deco(1,kc) = deco(1,kc) - dedxc
                  deco(2,kc) = deco(2,kc) - dedyc
                  deco(3,kc) = deco(3,kc) - dedzc
c
...
c
      ...
      einter = eintero
      do i = 1, n 
         dec(1,i) = dec(1,i) + deco(1,i)
         dec(2,i) = dec(2,i) + deco(2,i)
         dec(3,i) = dec(3,i) + deco(3,i)
      end do
      do i = 1, 3 
         vir(1,i) = viro(1,i)
         vir(2,i) = viro(2,i)
